...
const STEPS = [
  {
    id: '01',
    title: 'SCOPE',
    text: 'We translate the brief into a real plan: dates, deliverables, dependencies, and decision points. Nothing starts until the plan is signed off.'
  },
  {
    id: '02',
    title: 'STAFF',
    text: 'Internal team assigned. External vendors, if any, briefed, coordinated, and scheduled.'
  },
  {
    id: '03',
    title: 'SHIP',
    text: 'Weekly working sessions. Clear ownership. Vendors managed by us. You approve at decision points, not every step.'
  },
  {
    id: '04',
    title: 'MEASURE',
    text: 'After launch, a rigorous read of what the project actually accomplished against what we committed to do.'
  }
]
...
export default function BuildProcess() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      const steps = gsap.utils.toArray('.build-process-step') as HTMLElement[]
      ...
      steps.forEach((step, i) => {
        const isEven = i % 2 === 0
        const content = step.querySelector('.build-step-content')
        const marker = step.querySelector('.build-step-marker')
      ...
      <div className="relative z-20 w-full h-full max-w-[1400px] mx-auto flex items-center justify-center">
        ...
        <div className="relative w-full h-[60%] flex flex-col justify-between py-10">
          {STEPS.map((step, i) => (
            <div 
              key={step.id}
              className={`build-process-step relative w-full flex items-center ${i % 2 === 0 ? 'lg:justify-start' : 'lg:justify-end'} justify-start pl-16 lg:pl-0`}
            >
              {/* Central Spine Node */}
              <div className="build-step-marker absolute left-8 lg:left-1/2 lg:-translate-x-1/2 w-3.5 h-3.5 rounded-full z-20 border border-black" />

              {/* Step Card Content */}
              <div className={`build-step-content w-[85%] lg:w-[42%] ${i % 2 === 0 ? 'lg:pr-12 lg:text-right' : 'lg:pl-12 lg:text-left'} space-y-1 lg:space-y-4`}>
