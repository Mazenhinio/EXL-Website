...
export default function GrowWhoItsFor() {
  ...
  useEffect(() => {
    const loadGsap = async () => {
      ...
      const ctx = gsap.context(() => {
        ...
        gsap.set('.grow-laser-line', { scaleY: 0, transformOrigin: 'top center' })
        gsap.set('.grow-left-content', { opacity: 0, x: -60 })
        gsap.set('.grow-right-content', { opacity: 0, x: 60 })

        // 1. Draw the central Chartreuse laser line
        tl.to('.grow-laser-line', { scaleY: 1, duration: 2, ease: 'power2.inOut' })
        ...
        tl.to('.grow-left-content', { opacity: 1, x: 0, duration: 1.5, ease: 'power2.out' }, '-=1.2')
          .to('.grow-right-content', { opacity: 1, x: 0, duration: 1.5, ease: 'power2.out' }, '<')
  ...
  return (
    ...
    >
      ...
      <div 
        className="grow-laser-line absolute top-0 bottom-0 left-1/2 -translate-x-1/2 w-[2px] bg-[var(--chartreuse)] shadow-[0_0_15px_#DEFF00] z-30 pointer-events-none" 
      />
      ...
      >
        ...
        <div className="grow-left-content relative z-20 w-full max-w-2xl ml-auto mr-0 px-8 lg:px-14 flex flex-col justify-center">
          ...
          <div className="flex flex-col gap-5">
            {[
              'Brands with a real offer, a working product, and a marketing function ready to be accountable to pipeline.',
              'Companies with enough content to distribute and enough infrastructure to measure.',
              'Marketing leaders who are done with vanity metrics and ready to invest in the systems that generate revenue.'
            ].map((text, idx) => (
      ...
      >
        ...
        <div className="grow-right-content relative z-25 w-full max-w-2xl mr-auto ml-0 px-8 lg:px-14 flex flex-col justify-center">
          ...
          <div className="flex flex-col gap-5">
            {[
              'Brands still figuring out their positioning or core audience. (In those cases, we recommend Advise).',
              'Companies without a product worth distributing.',
              'A company looking for high-volume superficial vanity reporting rather than deep revenue attribution.'
            ].map((text, idx) => (
